 ...
 * Blocks for Kapakai
 */
//% weight=100 color=#A51F22 icon="\e04b" block="Kapakai"
namespace kapakai {
    export enum LedLr {
        //% block="A button"
        AButtonSide = 8,
     ...
     * @param num - kapakai.LedLr.AButtonSide or BButtonSide 
    */
    //% block="Init Mic %num"
    //% weight=77
    ...
    /**
     * Get the angle in the direction that "Kapakai: bit" is facing
     */
    //% block
    //% weight=75
    ...
    //% blockId=Kapakai:bit_motion_basic
    //% block="play basic motion %fileName"
    //% weight=90
    export function basicMotion(fileName: BasicMotions) {
        motion(fileName);
    }
    //% blockId=Kapakai:bit_motion_Futbol
    //% block="play futbol motion %fileName"
    //% weight=89
    export function futbolMotion(fileName: FutbolMotions) {
        motion(fileName);
    }
    // blockId=Kapakai:bit_motion_box
    // block="play delivery motion %fileName"
    //% advanced=true
    export function deliveryMotion(fileName: DeliveryMotions) {
        motion(fileName);
    }
    //% blockId=Kapakai:bit_motion_dance
    //% block="play dance motion %fileName"
    //% weight=88
    export function danceMotions(fileName: DanceMotions) {
        motion(fileName);
    }
    // blockId=Kapakai:bit_motion_m
    // block="play move motion %fileName"
    export function moveMotions(fileName: MoveMotions) {
        motion(fileName);
    }
    ...
    //% block="walk %mode"
    //% weight=85
    export function walking(mode: WalkMode){
        ...
        switch(modeNum){
            case 0:
                motionFlame(BasicMotions.WalkForward, 0);
                motionFlame(BasicMotions.WalkForward, 1);
                modeNum = 1;
                //break;
            case 1:
                motionFlame(BasicMotions.WalkForward, 2);
                motionFlame(BasicMotions.WalkForward, 3);
                motionFlame(BasicMotions.WalkForward, 4);
                motionFlame(BasicMotions.WalkForward, 5);
                motionFlame(BasicMotions.WalkForward, 6);
                motionFlame(BasicMotions.WalkForward, 7);
                break;
            case 2:
                motionFlame(BasicMotions.WalkForward, 8);
                motionFlame(BasicMotions.WalkForward, 9);
                modeNum = 0;
                break;
            default:
                break;
    ...
    function doMotion(fileName: number, flameNum: number) {
        ...
        let listLen = 43;
        let loopBool = false;
        let kapakaiDebug :boolean = false;
        if(flameNum == 0xff){
            flameNum = 0;
            loopBool = true;
        }
        ...
        while (1) {
            ...
            if(kapakaiDebug)serial.writeString(",>OK");
            listNum += 1; // >
            if ( 0x4d46 != ( mBuf[listNum] << 8 | mBuf[listNum + 1] ) )break;
            if(kapakaiDebug)serial.writeString(",mfOK");
            listNum += 2; // MF
            if ( fileName != ( num2Hex(mBuf[listNum]) << 4 | num2Hex(mBuf[listNum + 1]) ) )break;
            if(kapakaiDebug)serial.writeString(",fileOK");
            ...
            for (let val = 0; val < SERVO_NUM_USED; val++){
                ...
                data[val] = numHex;
                if(kapakaiDebug)serial.writeNumber(data[val]);
                listNum += 4;
            }
            setAngle(data, time);
            if(!loopBool)break;
         ...
         rs:number, rt:number, ra:number, rf:number, msec: number)
    {
        ...
        for (let n=0; n<8; n++){
            if(angle[n] >= 1800 - kapakai.servoSetInit[n]){
                angle[n] = 1800 - kapakai.servoSetInit[n];
            }else if(angle[n] <= (-1800) * kapakai.servoSetInit[n]){
                angle[n] = (-1800) * kapakai.servoSetInit[n];
            }
        }
        setAngle(angle, msec);
    ...
    export function setAngle(angle: number[], msec: number) {
        ...
        for (let i = 0; i <= msec; i++) {
            ...
            for (let val = 0; val < SERVO_NUM_USED; val++) {
                ...
                servoWrites(val, (servoAngle[val] / 10));
    ...
    export function servoAdjust(servoNum: number, adjustNum: number) {
        ...
        if (100 > adjNum) {
            ...
            servoWrites(servoNum, (adjNum / 10));
            basic.pause(0.5);
        }
        return adjustNum;
    }
    ...
    export function servoInitialSet() {
        for (let n = 0; n < 8; n++) {
            servoWrites(n, servoSetInit[n] / 10);
        }
    }
